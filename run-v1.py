...
class MemoryEfficientPatchcore(Patchcore):
    """Memory-efficient version of PatchCore that processes data in chunks"""
    
    def __init__(self, *args, chunk_size=4, **kwargs):
        super().__init__(*args, **kwargs)
        self.chunk_size = chunk_size
    
    def configure_optimizers(self):
        """Override to use memory-efficient optimizer settings"""
        # PatchCore doesn't typically need optimizers (it's not trained end-to-end)
        # But we can configure memory-efficient settings here if needed
        return None
...
def main():
    ...
    if memory_efficient:
        ...
        torch.backends.cudnn.benchmark = False  # Reduces memory fragmentation
        
        # Use smaller effective batch size through gradient accumulation
        effective_batch_size = args.train_batch_size
        actual_batch_size = min(chunk_size, args.train_batch_size)
        accumulate_grad_batches = effective_batch_size // actual_batch_size
        
        print(f"   📦 Chunking: {effective_batch_size} → {actual_batch_size} (accumulate {accumulate_grad_batches})")
        
        # Override batch sizes for memory efficiency
        args.train_batch_size = actual_batch_size
        args.eval_batch_size = min(chunk_size, args.eval_batch_size)
    ...
    # Use memory-efficient model if requested
    if memory_efficient:
        print("   🔧 Using memory-efficient PatchCore")
        model = MemoryEfficientPatchcore(
            backbone=args.backbone,
            layers=args.layers,
            pre_trained=True,
            coreset_sampling_ratio=args.coreset_sampling_ratio,
    ...
    # Configure engine with memory optimizations
    engine_kwargs = {
        "max_epochs": 1,
        "accelerator": "auto",
        "devices": 1,
    ...
    if memory_efficient:
        engine_kwargs.update({
            ...
            "enable_model_summary": False,  # Saves memory during setup
        })
        
        if use_mixed_precision:
            engine_kwargs["precision"] = "16-mixed"  # Use half precision
